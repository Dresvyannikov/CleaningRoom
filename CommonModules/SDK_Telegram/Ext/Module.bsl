				...
				Если НовоеСообщение.Свойство("message") И НовоеСообщение.message.Свойство("chat") Тогда
					...
					ИначеЕсли НовоеСообщение.message.text = "/NewOrder" Тогда
						...
						ИначеЕсли НовоеСообщение.message.text = "/MyOrder" Тогда
							...
							СпивокЗаказов = ПолучитьСписокЗаказов(id);
							sendMessage(id, "Список заказов на текущий месяц:"+Символы.ПС+СпивокЗаказов);
...
Функция СоздатьЗаказ(НовоеСообщение)  
	...
	НовыйЗаказ.ТелефонЗаказчика = Сред(ТекстСообщения,СтрНайти(НовоеСообщение.message.text,"1. ")+3,СтрНайти(ТекстСообщения,"2. ")-5);
	НовыйЗаказ.Адрес = Сред(ТекстСообщения,СтрНайти(НовоеСообщение.message.text,"2. ")+3,СтрНайти(ТекстСообщения,"3. ")-СтрНайти(ТекстСообщения,"2. ")-4);
...
Функция ПолучитьСписокЗаказов(ЧатИд) 
	ОбщийСписок =  "";
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		...
		|	ВТ_заказы.ОбщаяСтоимость КАК ОбщаяСтоимость
		|ИЗ
	...
	Пока Выборка.Следующий() Цикл
		ОбщийСписок =ОбщийСписок+" "+Строка(Выборка.Номер)+" "+Строка(Выборка.Ссылка)+" "+Строка(Выборка.СтатусЗаказа)+" "+Строка(Выборка.ОбщаяСтоимость)+" руб "+Символы.ПС; 
	КонецЦикла;  
	Возврат ОбщийСписок;
КонецФункции
